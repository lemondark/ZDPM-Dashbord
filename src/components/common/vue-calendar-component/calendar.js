  ...
  */
  ...
  getLeftArr: function (date) {
    ...
    for (let i = 0; i < leftNum; i++) {
      const nowTime = preDate.getFullYear() + '-' + ('00' + (preDate.getMonth() + 1)).slice(-2) + '-' + ('00' + (num + i)).slice(-2);
      arr.push({
        id: num + i,
        date: nowTime,
        isToday: false,
  ...
  getRightArr: function (date) {
    ...
    const _length = leftLength >= 35 ? 7 - leftLength % 7 : 14 - leftLength % 7;
    for (let i = 0; i < _length; i++) {
      const nowTime = nextDate.getFullYear() + '-' + ('00' + (nextDate.getMonth() + 1)).slice(-2) + '-' + ('00' + (i + 1)).slice(-2);
      arr.push({
        id: i + 1,
        date: nowTime,
        isToday: false,
  ...
  getMonthListNoOther: function (date) {
    ...
    for (let i = 0; i < num; i++) {
      const nowTime = year + '-' + month + '-' + ('00' + (i + 1)).slice(-2);
      arr.push({
        id: i + 1,
        date: nowTime,
